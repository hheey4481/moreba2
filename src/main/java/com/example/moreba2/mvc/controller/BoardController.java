...
import com.example.moreba2.mvc.model.dao.BoardDAO;
import com.example.moreba2.mvc.model.dao.RippleDAO;
import com.example.moreba2.mvc.model.dto.BoardDTO;
import com.example.moreba2.mvc.model.dto.RippleDTO;
...
//        }
//        else if(command.contains("/LikeUpdateAction.do")){
//            requestLikeUpdate(req, resp);
//        }
        else if (command.contains("/HotListAction.do")) {//등록된 글 목록 페이지 출력하기
            requestHotList(req);   //메소드 실행
            RequestDispatcher rd = req.getRequestDispatcher("../board/hotList.jsp");
            //RequestDispatcher rd객체가 가지고 있는 path에 해당하는 페이지로 이동
            //객체를 생성후 forward() 메소드를 이용하여 이동
            rd.forward(req, resp);
        }
    ...
    private void requestHotList(HttpServletRequest req) {   //핫게 목록
...
//
        hotlist = dao.getHotList(pageNum, limit, items, text);   //현재 페이지에 해당하는 목록 데이터 들고 오기
        int total_record = dao.getHotCount(items, text);
        ...
        req.setAttribute("hotlist", hotlist);      //현재 페이지에 해당하는 목록 데이터
...
// 폼 페이지에서 전송된 요청 파라미터가 일반 데이터이면 요청 파라미터의 이름과 값을 출력.
    ...
    //선택된 글 신고
    private void requestBoardAccuss(HttpServletRequest req) {
        int num=Integer.parseInt(req.getParameter("num"));
        int pageNum = Integer.parseInt(req.getParameter("pageNum"));
    ...
    //좋아요
//    private void requestBoardFavorite(HttpServletRequest req) throws IOException {
//
//        req.setCharacterEncoding ("UTF-8");
//        int num = Integer.parseInt(req.getParameter("num"));
//        HttpSession session = req.getSession();
...
//    }
    ...
    //좋아요 검색
//    private void requestBoardFavoriteSearch(HttpServletRequest req) {
//    }


//    private void requestLikeUpdate(HttpServletRequest req, HttpServletResponse resp) throws IOException {
