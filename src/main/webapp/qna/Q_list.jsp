...
<div class="container">
    <form action="<c:url value="./QnaListAction.ro"/>" method="post">
        <div>
        ...
        <div style="padding-top: 50px">
            <table class="table table-hover">
                ...
                <tr>
                    <td><%=serialNumber--%></td>
                    <td><%=notice.getMal()%></td>
                    <td><a href="./QnaViewAction.ro?num=<%=notice.getNum()%>&pageNum=<%=pageNum%>"><%=notice.getSubject()%></a></td>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination pagination-sm mt-3">
                    <li class="page-item"><a class="page-link"
                                             href="<c:url value="./QnaListAction.ro?pageNum=1${para}"/>">첫 페이지</a></li>
                    <c:if test="${thisBlock > 1}">
                        <li class="page-item"><a class="page-link" href="<c:url value="./QnaListAction.ro?pageNum=${firstPage - 1}${para}" />">이전</a></li>
                    </c:if>
                    <c:forEach var="i" begin="<%=firstPage %>" end="${lastPage}">
                        <c:choose>
                            <c:when test="${pageNum==i}">
                                <a class="page-link active"
                                   href="<c:url value="QnaListAction.ro?pageNum=${i}${para}"/>">
                                        ${i} </a>
                            </c:when>
                            <c:otherwise>
                                <a class="page-link"
                                   href="<c:url value="./QnaListAction.ro?pageNum=${i}${para}"/>">${i}</a>
                            </c:otherwise>
                        </c:choose>

                    </c:forEach>
                    <c:if test="${thisBlock < totalBlock}">
                        <li class="page-item"><a class="page-link"  href="<c:url value="./QnaListAction.ro?pageNum=${lastPage + 1}${para}" />">다음</a></li>
                    </c:if>
                    <c:if test="${total_page != pageNum}">
                        <li class="page-item"><a class="page-link"  href="<c:url value="./QnaListAction.ro?pageNum=${total_page}${para}" />">마지막 페이지</a></li>
                    </c:if>
                </ul>
            </nav>
        </div>
        <div align="center" class="mt-5" >
            <select  name="items" id="select">
                <option value="subject">제목에서</option>
                <option value="content">본문에서</option>
                <option value="name">글쓴이에서</option>
            </select> <input name="text" type="text" style="width: 30%" value="${text}"/>
...
<script type="text/javascript">
    function checkForm() {
        ...
        location.href = "./QnaWriteForm.ro?id=<%=sessionId%>"
    }
</script>
